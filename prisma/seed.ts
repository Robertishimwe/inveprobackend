...
// const SEED_TENANT_NAME = process.env.SEED_TENANT_NAME || 'Default Tenant';
// const SEED_ADMIN_EMAIL = process.env.SEED_ADMIN_EMAIL || 'admin@example.com';
// const SEED_ADMIN_PASSWORD = process.env.SEED_ADMIN_PASSWORD || 'StrongP@ssw0rd!';
// const SEED_ADMIN_FIRSTNAME = process.env.SEED_ADMIN_FIRSTNAME || 'Admin';
// const SEED_ADMIN_LASTNAME = process.env.SEED_ADMIN_LASTNAME || 'User';

// const ADMIN_ROLE_NAME = 'Admin';
// const ADMIN_ROLE_SYSTEM = true;

// // --- Core Permissions Definition (Keep the full list) ---
// const CORE_PERMISSIONS = [
//     // --- Core / General ---
//     { permissionKey: 'system:config:read', description: 'View system-level configurations' },
//     { permissionKey: 'system:config:update', description: 'Modify system-level configurations (Super Admin)' },
...
//     if(permissionIdsToAssign.length !== permissionKeys.length) {
//         const assignedKeys = CORE_PERMISSIONS.filter(cp => permissionIdsToAssign.includes(cp.permissionKey)).map(cp=>cp.permissionKey); // Find keys we actually found IDs for
//         const missingKeys = permissionKeys.filter(key => !assignedKeys.includes(key));
//         console.warn(`⚠️ Could not find all permissions for role '${roleName}'. Missing keys: ${missingKeys.join(', ')}. Ensure they are in CORE_PERMISSIONS array.`);
//     }
...
//     // Set exactly the specified permissions (delete old, create new)
//     await prisma.$transaction([
//         // Delete existing permissions for this role
//         prisma.rolePermission.deleteMany({ where: { roleId: role.id } }),
//         // Create new permissions if any were found
//         ...(permissionIdsToAssign.length > 0 ? [prisma.rolePermission.createMany({
//             data: permissionIdsToAssign.map(permissionId => ({
//                 roleId: role.id,
//                 permissionId: permissionId,
...
//         )
//     );
//     const allPermissions = await prisma.permission.findMany(); // Get all for Admin role later
//     console.log(`✅ ${allPermissions.length} core permissions ensured.`);

//     // 2. Create or Find the Default Tenant
//     console.log(`Ensuring tenant '${SEED_TENANT_NAME}' exists...`);
//     const tenant = await prisma.tenant.upsert({
//         where: { name: SEED_TENANT_NAME }, // Assuming name is unique for tenants, otherwise use a different unique field
//         update: {}, // No updates needed if tenant exists
//         create: { name: SEED_TENANT_NAME, status: 'ACTIVE' },
//     });
//     console.log(`✅ Tenant '${tenant.name}' ensured with ID: ${tenant.id}`);
...
//     // --- Add 4 More Common Roles ---

//     // Manager Role
//     const managerPermissionKeys = [
//         'dashboard:view', 'user:create', 'user:read:any', 'user:update:any', 'user:update:activity', 'user:assign:roles',
//         'role:read', // Can view roles to assign them
//         'category:create', 'category:read', 'category:update', 'category:delete',
//         'product:create', 'product:read', 'product:update', 'product:delete', 'product:assign:category',
//         'location:read', 'location:update', // Can update own store info maybe?
//         'inventory:read:levels', 'inventory:read:transactions', 'inventory:adjust', 'inventory:transfer:create', 'inventory:transfer:read', 'inventory:count:start', 'inventory:count:enter', 'inventory:count:review', 'inventory:count:approve',
//         'supplier:read', 'po:create', 'po:read', 'po:update', 'po:approve', 'po:receive', 'po:cancel',
//         'customer:create', 'customer:read', 'customer:update', 'customer:delete', 'group:read', 'customer:assign:group',
//         'order:create', 'order:read:any', 'order:update', 'order:update:status', 'order:cancel', 'order:manage:returns',
//         'pos:session:read:any', 'pos:session:reconcile', // Can view all sessions and reconcile
//         'report:view:sales', 'report:view:inventory', 'report:view:pos', 'report:view:purchasing', 'report:view:customer', 'report:export',
//         'template:manage',
//     ];
...
//     // Cashier/Sales Associate Role
//     const cashierPermissionKeys = [
//         'dashboard:view', 'user:read:own', 'user:update:own', 'user:update:password:own', // Manage self
//         'product:read', 'customer:read', 'customer:create', // Read products/customers, create new customers
//         'inventory:read:levels', // Check stock levels
//         'order:read:own', // View own orders/sales
//         'pos:session:start', 'pos:session:end', 'pos:session:read:own', 'pos:session:cash', // Manage own till
//         'pos:checkout', 'pos:return', // Perform sales and returns
//         'pos:discount:apply', // Maybe allow applying standard discounts?
//     ];
...
//          'location:read', // View location details (e.g., bin locations)
//          'inventory:read:levels', 'inventory:read:transactions', 'inventory:adjust', // Adjust stock (e.g., damages)
//          'inventory:transfer:create', 'inventory:transfer:ship', 'inventory:transfer:receive', 'inventory:transfer:read', 'inventory:transfer:cancel', // Manage transfers
//          'inventory:count:start', 'inventory:count:enter', // Perform stock counts
//          'inventory:manage:serials', 'inventory:manage:lots', // Manage tracked items
//          'po:read', 'po:receive', // View POs and receive stock
//     ];
...
//     // Read Only Analyst Role
//     const analystPermissionKeys = [
//         'dashboard:view', 'user:read:own', // View self
//         'category:read', 'product:read', 'location:read',
//         'inventory:read:levels', 'inventory:read:transactions', 'inventory:adjust:read', 'inventory:transfer:read', 'inventory:valuation:read',
//         'supplier:read', 'po:read',
//         'customer:read', 'group:read',
...
//         where: { email: SEED_ADMIN_EMAIL.toLowerCase() }, // Assuming email is globally unique for upsert
//         update: { // If admin exists, ensure they have the admin role for this tenant
//              roles: { connectOrCreate: { where: { userId_roleId: { userId: '', roleId: adminRole.id } }, create: { roleId: adminRole.id } } }, // Need user ID here, complex upsert needed if email not globally unique
//              // Simpler if email IS globally unique: ensure isActive, update names?
//              isActive: true,
...
//             lastName: SEED_ADMIN_LASTNAME,
//             isActive: true,
//             tenantId: tenant.id, // Associate with the created/found tenant
//             roles: { create: [{ role: { connect: { id: adminRole.id } } }] } // Assign Admin role
//         },
//     });
//      // Note: The upsert logic for ensuring the admin user has the role in the *correct tenant* if the user (email) already exists is tricky.
//      // It might be safer to just *create* the admin user if not found specifically for this tenant using findFirst + create, as done in the original script.
//      // Let's revert to the findFirst + create logic for clarity and safety.

//     // 4. (Revised) Create the Initial Admin User Safely for this Tenant
//     console.log(`(Revised) Ensuring admin user '${SEED_ADMIN_EMAIL}' exists for tenant ${tenant.id}...`);
//     let adminUser = await prisma.user.findFirst({
//         where: { email: SEED_ADMIN_EMAIL.toLowerCase(), tenantId: tenant.id },
//     });
//     if (!adminUser) {
...
//                  firstName: SEED_ADMIN_FIRSTNAME, lastName: SEED_ADMIN_LASTNAME,
//                  isActive: true, tenantId: tenant.id,
//                  roles: { create: [{ roleId: adminRole.id }] } // Simpler create syntax for join table
//              },
//          });
//          console.log(`✅ Admin user '${adminUser.email}' created for tenant ${tenant.id}`);
//     } else {
//          console.log(`✅ Admin user '${adminUser.email}' already exists for tenant ${tenant.id}. Ensuring Admin role assignment...`);
//          // Ensure the existing user has the admin role for this tenant
//          await prisma.userRole.upsert({
//              where: { userId_roleId: { userId: adminUser.id, roleId: adminRole.id } },
//              create: { userId: adminUser.id, roleId: adminRole.id },
//              update: {}
//          });
//          console.log(`✅ Ensured Admin role assignment for existing user.`);
//     }
