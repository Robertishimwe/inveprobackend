...
const config_1 = require("@/config"); // Import centralized Prisma client
const ApiError_1 = __importDefault(require("@/utils/ApiError"));
const logger_1 = __importDefault(require("@/utils/logger"));
const client_1 = require("@prisma/client"); // Import necessary Prisma types
/**
...
const createUser = async (userData, tenantId) => {
    ...
    const logContext = { function: 'createUser', email: lowerCaseEmail, tenantId };
    ...
    if (existingUser) {
        logger_1.default.warn(`User creation failed: Email already exists`, logContext);
        throw new ApiError_1.default(http_status_1.default.CONFLICT, 'Email address already in use by another user in this tenant.');
    }
    // 2. Check if provided roleIds are valid for the tenant
    let validRoleIds = [];
    if (userData.roleIds && userData.roleIds.length > 0) {
        const validRoles = await config_1.prisma.role.findMany({
        ...
        if (validRoles.length !== userData.roleIds.length) {
            ...
            logger_1.default.warn(`User creation failed: Invalid role ID(s) provided for this tenant`, logContext);
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Invalid role ID(s) provided: ${invalidIds.join(', ')}`);
        }
    ...
    try {
        const createdUserWithRoles = await config_1.prisma.user.create({
            data: {
                tenantId: tenantId,
        ...
        logger_1.default.info(`User created successfully`, logContext);
        return createdUserWithRoles;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error creating user in database`, logContext);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
            throw new ApiError_1.default(http_status_1.default.CONFLICT, 'Email address already in use.'); // More specific error if constraint is on email
        }
 ...
 */
const queryUsers = async (filter, orderBy, limit, page) => {
    const skip = (page - 1) * limit;
    const tenantIdForLog = typeof filter.tenantId === 'string' ? filter.tenantId : undefined;
    const logContext = { function: 'queryUsers', filter: '...', orderBy, limit, page, tenantId: tenantIdForLog };
    if (!tenantIdForLog) {
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Tenant context missing.');
    }
    try {
        const [users, totalResults] = await config_1.prisma.$transaction([
            ...
            config_1.prisma.user.count({ where: filter }),
        ]);
        logger_1.default.debug(`User query successful, found ${users.length} of ${totalResults} users.`, logContext);
        return { users: users, totalResults };
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error querying users`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve users.');
    }
};
/**
...
const getUserById = async (userId, tenantId) => {
    const logContext = { function: 'getUserById', userId, tenantId };
    try {
        ...
        if (!user) {
            logger_1.default.warn(`User not found or tenant mismatch`, logContext);
            return null;
        }
        logger_1.default.debug(`User found successfully`, logContext);
        return user; // Cast to appropriate safe type
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error fetching user by ID`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve user.');
    }
};
/**
 ...
 */
const updateUserById = async (userId, updateData, // Should NOT contain roleIds
tenantId) => {
    const logContext = { function: 'updateUserById', userId, tenantId, updateData: { ...updateData } };
    // 1. Verify user exists within the tenant first
    const existingUserCheck = await config_1.prisma.user.count({ where: { id: userId, tenantId: tenantId } });
    if (!existingUserCheck) {
        logger_1.default.warn(`Update failed: User not found or tenant mismatch`, logContext);
    ...
    try {
        ...
        logger_1.default.info(`User updated successfully`, logContext);
        return updatedUser;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error updating user`, logContext);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'User not found during update attempt.');
        }
...
const assignRoleToUser = async (userId, roleId, tenantId) => {
    const logContext = { function: 'assignRoleToUser', userId, roleId, tenantId };
    try {
        ...
        logger_1.default.info(`Role ${roleId} assigned successfully to user ${userId}`, logContext);
    }
    catch (error) {
        if (error instanceof ApiError_1.default)
            throw error;
        logContext.error = error;
        logger_1.default.error(`Error assigning role to user`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to assign role.');
    }
};
/**
...
const removeRoleFromUser = async (userId, roleId, tenantId) => {
    const logContext = { function: 'removeRoleFromUser', userId, roleId, tenantId };
    // Verify user exists first for better error message
    const userExists = await config_1.prisma.user.count({ where: { id: userId, tenantId } });
    if (!userExists)
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'User not found.');
    try {
        const deleteResult = await config_1.prisma.userRole.deleteMany({
            where: { userId: userId, roleId: roleId } // Target specific assignment
        });
        if (deleteResult.count === 0) {
            logger_1.default.warn(`Role assignment not found for user ${userId} and role ${roleId}. No action taken.`, logContext);
            // Don't throw error, operation is idempotent
        }
        else {
            logger_1.default.info(`Role ${roleId} removed successfully from user ${userId}`, logContext);
        }
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error removing role from user`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to remove role.');
    }
};
/**
 * Soft delete a user by ID (mark as inactive).
 */
const deleteUserById = async (userId, tenantId, requestingUserId) => {
    const logContext = { function: 'deleteUserById', userId, tenantId, requestingUserId };
    if (requestingUserId === userId) {
        logger_1.default.warn(`User attempted self-deletion`, logContext);
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Users cannot deactivate their own account.');
    }
    try {
        const result = await config_1.prisma.user.updateMany({
            where: { id: userId, tenantId: tenantId, isActive: true },
            data: { isActive: false },
        });
        if (result.count === 0) {
            const exists = await config_1.prisma.user.findFirst({ where: { id: userId, tenantId: tenantId }, select: { isActive: true } });
            if (!exists) {
                logger_1.default.warn(`Soft delete failed: User not found or tenant mismatch`, logContext);
                throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'User not found.');
            }
            else {
                logger_1.default.info(`User was already inactive, no action taken`, logContext);
                return; // Success if already inactive
            }
        }
        // Revoke refresh tokens
        const revokeResult = await config_1.prisma.refreshToken.updateMany({
            where: { userId: userId, revokedAt: null },
            data: { revokedAt: new Date() }
        });
        logContext.refreshTokensRevoked = revokeResult.count;
        logger_1.default.info(`User soft deleted (marked inactive) and revoked ${revokeResult.count} refresh tokens successfully`, logContext);
    }
    catch (error) {
        if (error instanceof ApiError_1.default)
            throw error;
        logContext.error = error;
        logger_1.default.error(`Error during user soft delete`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to deactivate user.');
    }
};
exports.userService = {
    createUser,
