...
const config_1 = require("@/config"); // Centralized Prisma client
...
const pick_1 = __importDefault(require("@/utils/pick")); // Import pick utility
...
async function generatePONumber(tenantId, tx) {
    ...
    try {
        ...
        const result = await tx.$queryRawUnsafe(`SELECT nextval('"${sequenceName}"')`); // Use quotes for safety if seq name needs it
        if (!result || result.length === 0 || typeof result[0]?.nextval !== 'bigint') {
            logger_1.default.error(`Unexpected result structure from sequence query for ${sequenceName}`, { result });
        ...
        if (poNumExists) {
            ...
            logger_1.default.error(`Generated PO Number ${poNumber} already exists despite using sequence!`, { tenantId });
            throw new ApiError_1.default(http_status_1.default.CONFLICT, `Generated PO Number ${poNumber} already exists (sequence error?).`);
        }
        return poNumber;
    }
    catch (seqError) {
        logger_1.default.error(`Error fetching PO number from sequence ${sequenceName}`, { tenantId, error: seqError });
        if (seqError?.code === '42P01') { // PostgreSQL code for undefined_table/sequence
            throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, `Database sequence "${sequenceName}" not found. Please ensure it exists.`);
        }
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, `Could not generate PO number from sequence.`);
    }
}
// --- Helper: Prepare Item Data & Calculate PO Totals ---
function prepareItemsAndCalculateTotals(itemsDto, shippingCostInput, tenantId // Needed if item model requires tenantId directly
) {
    let subtotal = new client_1.Prisma.Decimal(0);
    let totalTax = new client_1.Prisma.Decimal(0);
    const shippingCost = new client_1.Prisma.Decimal(shippingCostInput ?? 0);
    const itemsData = itemsDto.map(item => {
        if (!item.productId)
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Missing productId for an item.`);
        if (item.quantityOrdered <= 0)
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Quantity ordered for product ${item.productId} must be positive.`);
        if (item.unitCost === undefined || item.unitCost === null || item.unitCost < 0)
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Unit cost for product ${item.productId} is required and cannot be negative.`);
        const quantity = new client_1.Prisma.Decimal(item.quantityOrdered);
        const cost = new client_1.Prisma.Decimal(item.unitCost);
        const lineTotal = quantity.times(cost);
        const taxRate = item.taxRate ? new client_1.Prisma.Decimal(item.taxRate) : new client_1.Prisma.Decimal(0);
        if (taxRate.lessThan(0))
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Tax rate for product ${item.productId} cannot be negative.`);
...
async function _updateInventoryItemQuantity(tx, tenantId, productId, locationId, quantityChange) {
    const quantityChangeDecimal = new client_1.Prisma.Decimal(quantityChange);
    if (quantityChangeDecimal.isZero()) {
        // This case should ideally be filtered out before calling this helper
        logger_1.default.warn(`_updateInventoryItemQuantity called with zero quantity change for Product ${productId}, Location ${locationId}`);
    ...
    if (inventoryItem.quantityOnHand.lessThan(0)) {
        ...
        if (!allowNegativeStock) {
            ...
            logger_1.default.warn(`Stock quantity went negative for item ${inventoryItem.id} (Allowed by config).`);
        }
    }
...
const createPurchaseOrder = async (data, tenantId, userId) => {
    const logContext = { function: 'createPurchaseOrder', tenantId, userId, supplierId: data.supplierId, locationId: data.locationId };
    // 1. Validate Supplier and Location (ensure active)
    const [supplier, location] = await Promise.all([
        config_1.prisma.supplier.findFirst({ where: { id: data.supplierId, tenantId, isActive: true }, select: { id: true } }),
        config_1.prisma.location.findFirst({ where: { id: data.locationId, tenantId, isActive: true }, select: { id: true } })
    ]);
    if (!supplier)
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Active supplier with ID ${data.supplierId} not found or inactive.`);
    if (!location)
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Active delivery location with ID ${data.locationId} not found or inactive.`);
    // 2. Validate Products (ensure active)
    const productIds = data.items.map(item => item.productId);
    if (productIds.length === 0) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Purchase order must contain at least one item.`);
    }
    const validProducts = await config_1.prisma.product.findMany({ where: { id: { in: productIds }, tenantId, isActive: true }, select: { id: true } });
    if (validProducts.length !== productIds.length) {
        const missingIds = productIds.filter(id => !validProducts.some(p => p.id === id));
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `One or more active products not found: ${missingIds.join(', ')}`);
    }
    // 3. Prepare Item Data and Calculate Totals
    const { itemsData, subtotal, taxAmount, totalAmount, shippingCost } = prepareItemsAndCalculateTotals(data.items, data.shippingCost, tenantId);
    // 4. Create PO and Items in Transaction
    try {
        const newPO = await config_1.prisma.$transaction(async (tx) => {
            ...
            if (data.poNumber && data.poNumber !== poNumber) { // If manual provided, check its uniqueness
                ...
                if (poNumExists) {
                    throw new ApiError_1.default(http_status_1.default.CONFLICT, `Manual PO Number ${data.poNumber} already exists.`);
                }
            ...
            return await tx.purchaseOrder.create({
                data: {
                    ...
                    status: client_1.PurchaseOrderStatus.DRAFT, // Start as DRAFT
                    orderDate: new Date(),
        ...
        logger_1.default.info(`Purchase order created successfully`, logContext);
        return newPO;
    }
    catch (error) {
        ...
        logger_1.default.error(`Error creating purchase order`, logContext);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
            // Unique constraint violation (likely poNumber if manual override was used and existed)
            throw new ApiError_1.default(http_status_1.default.CONFLICT, `PO Number conflict or other unique constraint violation during creation.`);
        }
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to create purchase order.');
    }
};
/** Query Purchase Orders */
const queryPurchaseOrders = async (filter, orderBy, limit, page) => {
    const skip = (page - 1) * limit;
    const tenantIdForLog = typeof filter.tenantId === 'string' ? filter.tenantId : undefined;
    const logContext = { function: 'queryPurchaseOrders', tenantId: tenantIdForLog, limit, page };
    if (!tenantIdForLog) {
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Tenant context missing.');
    }
    try {
        const [pos, totalResults] = await config_1.prisma.$transaction([
            config_1.prisma.purchaseOrder.findMany({
                where: filter,
                include: {
                    supplier: { select: { id: true, name: true } },
                    location: { select: { id: true, name: true } },
            ...
            config_1.prisma.purchaseOrder.count({ where: filter }),
        ]);
        logger_1.default.debug(`PO query successful, found ${pos.length} of ${totalResults}`, logContext);
        return { pos: pos, totalResults };
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error querying purchase orders`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve purchase orders.');
    }
};
/** Get Purchase Order By ID */
const getPurchaseOrderById = async (poId, tenantId) => {
    const logContext = { function: 'getPurchaseOrderById', poId, tenantId };
    try {
        const po = await config_1.prisma.purchaseOrder.findFirst({
        ...
        if (!po) {
            logger_1.default.warn(`PO not found or tenant mismatch`, logContext);
            return null;
        }
        logger_1.default.debug(`PO found successfully`, logContext);
        return po;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error fetching PO by ID`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve purchase order.');
    }
};
/** Update Basic Purchase Order Details (Allowed fields depend on status) */
const updatePurchaseOrder = async (poId, updateData, tenantId, userId) => {
    const logContext = { function: 'updatePurchaseOrder', poId, tenantId, userId, data: updateData };
    // Fetch full PO for calculations and status check
    const existingPO = await config_1.prisma.purchaseOrder.findFirst({ where: { id: poId, tenantId } });
    if (!existingPO) {
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Purchase Order not found.');
    }
    const isDraft = existingPO.status === client_1.PurchaseOrderStatus.DRAFT;
    ...
    const allowedUpdates = (0, pick_1.default)(updateData, allowedFields);
    if (Object.keys(allowedUpdates).length === 0) {
        logger_1.default.info(`PO update skipped: No allowed fields provided or no changes`, logContext);
        // Return full details
        return getPurchaseOrderById(poId, tenantId).then(po => { if (!po)
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'PO not found after skip.'); return po; });
    }
    const dataToUpdate = {};
    if (allowedUpdates.expectedDeliveryDate !== undefined)
        dataToUpdate.expectedDeliveryDate = allowedUpdates.expectedDeliveryDate ? new Date(allowedUpdates.expectedDeliveryDate) : null;
    if (allowedUpdates.notes !== undefined)
        dataToUpdate.notes = allowedUpdates.notes;
    ...
    if (allowedUpdates.shippingCost !== undefined) {
        const newShippingCost = new client_1.Prisma.Decimal(allowedUpdates.shippingCost);
    ...
    if (needsTotalRecalc) {
        ...
        const currentShippingCost = (dataToUpdate.shippingCost ?? existingPO.shippingCost);
    ...
    if (Object.keys(dataToUpdate).length === 0) {
        logger_1.default.info(`PO update skipped: No effective changes after status/value checks`, logContext);
        return getPurchaseOrderById(poId, tenantId).then(po => { if (!po)
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'PO not found after skip.'); return po; });
    }
    try {
        const updatedPO = await config_1.prisma.purchaseOrder.update({
            where: { id: poId },
            data: dataToUpdate,
            include: {
                supplier: { select: { id: true, name: true } },
        ...
        logger_1.default.info(`PO ${existingPO.poNumber} updated successfully`, logContext);
        return updatedPO;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error updating PO`, logContext);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'PO not found during update attempt.');
        }
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to update purchase order.');
    }
};
// --- PO Status Transition Functions ---
/** Helper to update PO Status, ensuring valid transitions */
const _updatePOStatus = async (poId, tenantId, userId, allowedFromStatuses, newStatus, notes) => {
    const logContext = { function: '_updatePOStatus', poId, tenantId, userId, newStatus, notes };
    const po = await config_1.prisma.purchaseOrder.findFirst({ where: { id: poId, tenantId }, select: { id: true, status: true, poNumber: true, notes: true } });
    if (!po)
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Purchase Order not found.');
    if (!allowedFromStatuses.includes(po.status)) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Cannot change PO status from ${po.status} to ${newStatus}.`);
    }
    if (po.status === newStatus) {
        logger_1.default.info(`PO ${po.poNumber} status is already ${newStatus}. No update performed.`, logContext);
        return getPurchaseOrderById(poId, tenantId).then(fullPo => { if (!fullPo)
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'PO not found'); return fullPo; });
    }
    try {
        const updatedPO = await config_1.prisma.purchaseOrder.update({
            where: { id: poId },
        ...
        logger_1.default.info(`PO ${po.poNumber} status updated to ${newStatus}`, logContext);
        // TODO: Trigger side effects (update 'incoming' stock, emails)
        return updatedPO;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error updating PO status`, logContext);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'PO not found during status update.');
        }
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to update PO status.');
    }
};
/** Submit PO for Approval (DRAFT -> PENDING_APPROVAL) */
const submitPurchaseOrder = async (poId, tenantId, userId, actionData) => {
    return _updatePOStatus(poId, tenantId, userId, [client_1.PurchaseOrderStatus.DRAFT], client_1.PurchaseOrderStatus.PENDING_APPROVAL, actionData?.notes);
};
/** Approve Purchase Order (DRAFT or PENDING_APPROVAL -> APPROVED) */
const approvePurchaseOrder = async (poId, tenantId, userId, actionData) => {
    return _updatePOStatus(poId, tenantId, userId, [client_1.PurchaseOrderStatus.DRAFT, client_1.PurchaseOrderStatus.PENDING_APPROVAL], client_1.PurchaseOrderStatus.APPROVED, actionData?.notes);
};
/** Mark Purchase Order as Sent (APPROVED -> SENT) */
const sendPurchaseOrder = async (poId, tenantId, userId, actionData) => {
    logger_1.default.info(`Simulating sending PO ${poId} to supplier... (Actual sending not implemented)`);
    return _updatePOStatus(poId, tenantId, userId, [client_1.PurchaseOrderStatus.APPROVED], client_1.PurchaseOrderStatus.SENT, actionData?.notes);
};
/** Cancel Purchase Order */
const cancelPurchaseOrder = async (poId, tenantId, userId, actionData) => {
    const cancellableStatuses = [client_1.PurchaseOrderStatus.DRAFT, client_1.PurchaseOrderStatus.PENDING_APPROVAL, client_1.PurchaseOrderStatus.APPROVED, client_1.PurchaseOrderStatus.SENT, client_1.PurchaseOrderStatus.PARTIALLY_RECEIVED];
    const reason = actionData?.notes ?? 'Cancelled by user';
    const po = await _updatePOStatus(poId, tenantId, userId, cancellableStatuses, client_1.PurchaseOrderStatus.CANCELLED, reason);
    logger_1.default.info(`PO ${po.poNumber} cancelled. Incoming stock reversal may be needed.`);
    return po;
};
...
//  */
const receivePurchaseOrderItems = async (poId, data, tenantId, userId) => {
    const logContext = { function: 'receivePurchaseOrderItems', poId, tenantId, userId };
    const startTime = Date.now(); // For performance logging
    // --- Pre-fetch and Validate Outside Transaction ---
    logger_1.default.debug(`Starting pre-fetch for PO ${poId}`, logContext);
    const poForCheck = await config_1.prisma.purchaseOrder.findUnique({
        where: { id: poId, tenantId: tenantId },
        include: {
            items: {
                include: {
    ...
    if (!poForCheck) {
        logger_1.default.warn(`PO Receiving failed: PO not found or tenant mismatch`, logContext);
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Purchase Order not found.');
    }
    logContext.poNumber = poForCheck.poNumber; // Add PO number to context
    // Validate PO status for receiving
    const allowedReceiveStatuses = [client_1.PurchaseOrderStatus.SENT, client_1.PurchaseOrderStatus.PARTIALLY_RECEIVED];
    if (!allowedReceiveStatuses.includes(poForCheck.status)) {
        logger_1.default.warn(`PO Receiving failed: Invalid status ${poForCheck.status}`, logContext);
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Cannot receive items against PO with status ${poForCheck.status}.`);
    }
    const poItemsMap = new Map(poForCheck.items.map(item => [item.id, item]));
    ...
    for (const receivedItemDto of data.items) {
        ...
        if (!poLineItem) {
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `PO Item ID ${receivedItemDto.poItemId} not found on PO ${poForCheck.poNumber}.`);
        }
        if (!poLineItem.product.isStockTracked) {
            logger_1.default.warn(`Skipping receive attempt for non-stock-tracked product`, itemLogContext);
            continue; // Skip non-tracked items from being processed further
        }
        const quantityReceivedDecimal = new client_1.Prisma.Decimal(receivedItemDto.quantityReceived);
        if (quantityReceivedDecimal.lessThanOrEqualTo(0)) {
            logger_1.default.warn(`Skipping zero/negative quantity received for PO Item`, itemLogContext);
        ...
        if (quantityReceivedDecimal.greaterThan(maxReceivable)) {
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Received quantity ${quantityReceivedDecimal} exceeds outstanding quantity ${maxReceivable} for PO Item ${poLineItem.id} (Product ${poLineItem.productId}).`);
        }
        ...
        if (poLineItem.product.requiresSerialNumber) {
            ...
            if (!quantityReceivedDecimal.isInteger()) {
                throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Received quantity (${quantityReceivedDecimal}) must be a whole number for serialized PO Item ${poLineItem.id}.`);
            }
            const expectedSerialCount = quantityReceivedDecimal.toNumber();
            if (serialsToReceive.length !== expectedSerialCount) {
                throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Incorrect number of serial numbers provided for PO Item ${poLineItem.id}. Expected ${expectedSerialCount}, got ${serialsToReceive.length}.`);
            }
    ...
    if (itemsToProcess.length === 0) {
        logger_1.default.warn(`PO receiving request contained no valid stock-tracked items with positive quantity to process for PO ${poForCheck.poNumber}.`, logContext);
    ...
    logger_1.default.debug(`Pre-fetch and validation completed for PO ${poId}. Duration: ${prefetchEndTime - startTime}ms`, logContext);
    // --- End Pre-fetch / Pre-validation ---
    // --- Transactional Operations ---
    try {
        const transactionStartTime = Date.now();
        // Use a longer timeout for potentially complex receiving transactions
        const updatedStatus = await config_1.prisma.$transaction(async (tx) => {
            ...
            for (const itemData of itemsToProcess) {
                ...
                if (poLineItem.product.requiresSerialNumber && serialsToReceive.length > 0) {
                    ...
                    for (const serial of serialsToReceive) {
                        ...
                        inventoryTransactionData.push({
                            tenantId, productId: poLineItem.productId, locationId: poForCheck.locationId,
                            transactionType: client_1.InventoryTransactionType.PURCHASE_RECEIPT,
                            quantityChange: new client_1.Prisma.Decimal(1), // Quantity is 1 per serial
                ...
                logger_1.default.debug(`Updated PO Item ${poLineItem.id} received quantity by ${quantityReceivedDecimal}`, itemLogContext);
            } // End loop through itemsToProcess
            ...
            if (inventoryTransactionData.length > 0) {
                ...
                logger_1.default.debug(`Batch created ${createdTxResult.count} inventory transactions.`, logContext);
            }
            else {
                // This case should ideally not be reached if itemsToProcess was not empty,
                // but log just in case.
                logger_1.default.warn(`No inventory transactions were generated during PO receiving.`, logContext);
            }
            ...
            const totalOrdered = updatedItems.reduce((sum, item) => sum.plus(item.quantityOrdered), new client_1.Prisma.Decimal(0));
            const totalReceived = updatedItems.reduce((sum, item) => sum.plus(item.quantityReceived), new client_1.Prisma.Decimal(0));
            let newStatus = poForCheck.status; // Start with current status from pre-fetch
            const tolerance = new client_1.Prisma.Decimal('0.00001'); // Tolerance for decimal comparison
        ...
        logger_1.default.info(`Items received successfully against PO ${poId}. Final Status: ${updatedStatus}. Tx Duration: ${transactionEndTime - transactionStartTime}ms`, logContext);
        return { success: true, updatedStatus: updatedStatus };
    }
    catch (error) {
        ...
        logger_1.default.error(`Error receiving purchase order items`, logContext);
        // Don't throw the original error directly if it's an internal DB issue leaking details
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, `Failed to receive PO items: ${error instanceof ApiError_1.default ? error.message : 'Internal processing error.'}`);
    }
};
