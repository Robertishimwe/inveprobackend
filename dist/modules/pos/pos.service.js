...
const getCurrentSession = async (userId, posTerminalId, locationId, tenantId) => {
    const logContext = { function: 'getCurrentSession', userId, posTerminalId, locationId, tenantId };
    try {
        const session = await config_1.prisma.posSession.findFirst({
            where: { userId, posTerminalId, locationId, tenantId, status: client_1.PosSessionStatus.OPEN }
        });
        logger_1.default.debug(`Current session lookup result: ${session ? 'Found' : 'Not Found'}`, logContext);
        return session;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error fetching current POS session`, logContext);
        // --- FIX: Throw from catch ---
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve current session.');
        // ---------------------------
    }
};
/** Start a new POS Session */
const startSession = async (data, userId, posTerminalId, locationId, tenantId) => {
    const logContext = { function: 'startSession', userId, posTerminalId, locationId, tenantId, startingCash: data.startingCash };
    const existingOpenSession = await getCurrentSession(userId, posTerminalId, locationId, tenantId);
    if (existingOpenSession) {
        logger_1.default.warn(`Start session failed: Session already open`, { ...logContext, sessionId: existingOpenSession.id });
        throw new ApiError_1.default(http_status_1.default.CONFLICT, 'An active session already exists for this user/terminal/location.');
    }
    try {
        const startingCashDecimal = new client_1.Prisma.Decimal(data.startingCash);
        const session = await config_1.prisma.$transaction(async (tx) => {
            ...
            if (startingCashDecimal.greaterThan(0)) {
                await tx.posSessionTransaction.create({
                    data: {
                        ...
                        transactionType: client_1.PosTransactionType.PAY_IN,
        ...
        logger_1.default.info(`POS session started successfully`, logContext);
        return session;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error starting POS session`, logContext);
        // --- FIX: Throw from catch ---
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to start session.');
        // ---------------------------
    }
};
/** End a POS Session */
...
//     }
// };
const endSession = async (sessionId, data, userId, posTerminalId, locationId, tenantId) => {
    const logContext = { function: 'endSession', sessionId, userId, posTerminalId, locationId, tenantId, endingCash: data.endingCash };
    try {
        const endedSession = await config_1.prisma.$transaction(async (tx) => {
            const session = await tx.posSession.findFirst({
                where: { id: sessionId, tenantId, userId, posTerminalId, locationId, status: client_1.PosSessionStatus.OPEN }
            });
            if (!session) {
                throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Active session not found for this user/terminal/location, or session ID is incorrect.');
            }
            ...
            transactions.forEach(t => {
                switch (t.transactionType) {
                    case client_1.PosTransactionType.PAY_IN:
                    case client_1.PosTransactionType.CASH_SALE:
                        calculatedCash = calculatedCash.plus(t.amount);
                        break;
                    case client_1.PosTransactionType.PAY_OUT:
                    case client_1.PosTransactionType.CASH_REFUND:
                        calculatedCash = calculatedCash.minus(t.amount);
                        break;
            ...
            // --- End of FIX ---
            const endingCashDecimal = new client_1.Prisma.Decimal(data.endingCash);
            const difference = endingCashDecimal.minus(calculatedCash);
            logContext.calculatedCash = calculatedCash.toNumber();
            logContext.difference = difference.toNumber();
        ...
        logger_1.default.info(`POS session ended successfully`, logContext);
        if (endedSession.difference && !endedSession.difference.isZero()) {
            logger_1.default.warn(`POS session ended with cash difference: ${endedSession.difference}`, logContext);
        }
        return endedSession;
    }
    catch (error) {
        if (error instanceof ApiError_1.default)
            throw error;
        logContext.error = error;
        logger_1.default.error(`Error ending POS session`, logContext);
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to end session.');
    }
};
/** Reconcile a CLOSED POS Session */
const reconcileSession = async (sessionId, tenantId) => {
    const logContext = { function: 'reconcileSession', sessionId, tenantId };
    const session = await config_1.prisma.posSession.findFirst({
        where: { id: sessionId, tenantId, status: client_1.PosSessionStatus.CLOSED }
    });
    if (!session)
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Closed session not found for reconciliation.');
    try {
        const reconciledSession = await config_1.prisma.posSession.update({
            where: { id: sessionId },
            data: { status: client_1.PosSessionStatus.RECONCILED }
        });
        logger_1.default.info(`POS session reconciled successfully`, logContext);
        return reconciledSession;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error reconciling POS session`, logContext);
        // --- FIX: Throw from catch ---
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Session not found during reconciliation update attempt.');
        }
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to reconcile session.');
        // ---------------------------
    }
};
/** Record Cash Pay In/Out */
const recordCashTransaction = async (sessionId, data, userId, tenantId) => {
    const logContext = { function: 'recordCashTransaction', sessionId, userId, tenantId, type: data.transactionType, amount: data.amount };
    const session = await config_1.prisma.posSession.findFirst({
        where: { id: sessionId, tenantId, userId, status: client_1.PosSessionStatus.OPEN },
        select: { id: true }
    });
    if (!session)
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Active session not found for this user to record cash transaction.');
    try {
        const amountDecimal = new client_1.Prisma.Decimal(data.amount);
        const cashTx = await config_1.prisma.posSessionTransaction.create({
            data: {
        ...
        logger_1.default.info(`Cash transaction recorded successfully`, logContext);
        return cashTx;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error recording cash transaction`, logContext);
        // --- FIX: Throw from catch ---
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to record cash transaction.');
        // ---------------------------
    }
};
// --- Checkout Process ---
...
const processPosCheckout = async (checkoutData, sessionId, // Session must be active
tenantId, userId, // Cashier ID from auth context
posTerminalId, locationId, ipAddress, // Optional for logging/token association
userAgent // Optional for logging/token association
) => {
    const logContext = { function: 'processPosCheckout', tenantId, userId, sessionId, locationId, terminalId: posTerminalId, customerId: checkoutData.customerId };
    ...
    if (!currentSession || currentSession.id !== sessionId) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Invalid or inactive POS session for this user/terminal/location.');
    }
    // 2. Fetch Products (ensure they exist, are active, and get stock levels)
    const productIds = checkoutData.items.map(item => item.productId);
    if (productIds.length === 0) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Checkout must include at least one item.');
    }
    ...
    if (products.length !== productIds.length) {
        ...
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Product IDs not found or inactive: ${missingIds.join(', ')}`);
    }
    // 3. Basic Payment Validation (Total amount check)
    if (!checkoutData.payments || checkoutData.payments.length === 0) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'At least one payment method is required for checkout.');
    }
    const totalPaymentAmount = checkoutData.payments.reduce((sum, p) => sum + p.amount, 0);
    if (totalPaymentAmount <= 0) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Total payment amount must be positive.');
    }
    // --- Transactional Operations ---
    try {
        const createdOrder = await config_1.prisma.$transaction(async (tx) => {
            ...
            const session = await tx.posSession.findFirst({ where: { id: sessionId, status: client_1.PosSessionStatus.OPEN }, select: { id: true } });
            if (!session)
                throw new Error("Session closed unexpectedly during transaction."); // Internal error state
            ...
            const orderItemsData = []; // Use type for nested create
            ...
            for (const itemDto of checkoutData.items) {
                ...
                if (!product)
                    throw new Error(`Consistency Error: Product ${itemDto.productId} not found during transaction.`);
                ...
                const unitPrice = itemDto.unitPrice !== undefined
                    ...
                    : product.basePrice ?? new client_1.Prisma.Decimal(0); // Default to 0 if basePrice is null
            ...
            // 5. Calculate Final Order Total
            const discountTotal = new client_1.Prisma.Decimal(checkoutData.discountAmount ?? 0);
            const shippingTotal = new client_1.Prisma.Decimal(0); // Typically zero for POS
            const taxTotal = new client_1.Prisma.Decimal(0); // TODO: Implement Tax Calculation Service/Logic
            const calculatedTotal = calculatedSubtotal.minus(discountTotal).plus(shippingTotal).plus(taxTotal);
            // 6. Validate Payment Total against Order Total
            const paymentTotalDecimal = new client_1.Prisma.Decimal(totalPaymentAmount);
            // Use tolerance for potential floating point issues if needed, but Decimal should be precise
            if (!paymentTotalDecimal.equals(calculatedTotal)) {
                throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, `Payment total (${paymentTotalDecimal}) does not match calculated order total (${calculatedTotal}). Please verify cart and payment amounts.`);
            }
            // 7. Generate Order Number
            const orderNumber = await order_service_1.orderService.generateOrderNumber(tenantId); // Consider if needs to be outside tx
            // 8. Create Order Header including nested Items and Payments
            const order = await tx.order.create({
                data: {
                    tenantId,
                    ...
                    orderType: client_1.OrderType.POS, // Explicitly POS
                    status: client_1.OrderStatus.COMPLETED, // POS orders usually completed immediately
                    orderDate: new Date(),
                    ...
                    shippingAddress: checkoutData.shippingAddress ?? client_1.Prisma.JsonNull,
                    ...
                    payments: {
                        create: checkoutData.payments.map(p => ({
                            ...
                            paymentMethod: p.paymentMethod,
                            amount: new client_1.Prisma.Decimal(p.amount),
                            currencyCode: 'USD', // Use order currency
                            status: client_1.PaymentStatus.COMPLETED, // Assume POS payments are completed
                            transactionReference: p.transactionReference,
                            paymentDate: new Date(),
            ...
            // 9. Log CASH payment(s) to POS Session Transaction log
            const cashPayments = checkoutData.payments.filter(p => p.paymentMethod === client_1.PaymentMethod.CASH);
            for (const cashPayment of cashPayments) {
                await tx.posSessionTransaction.create({
                    data: {
                        tenantId, // Add if needed
                        posSessionId: sessionId,
                        transactionType: client_1.PosTransactionType.CASH_SALE,
                        amount: new client_1.Prisma.Decimal(cashPayment.amount), // Positive amount for cash received
            ...
            for (const move of stockMovements) {
                ...
                await inventory_service_1.inventoryService._recordStockMovement(tx, tenantId, userId, move.productId, locationId, // Use order's locationId
                move.quantity.negated(), // Decrease stock (use negated Decimal)
                client_1.InventoryTransactionType.SALE, null, // Cost of Goods Sold calculation happens later if needed
        ...
        logger_1.default.info(`POS Checkout successful`, logContext);
        return createdOrder; // Cast transaction result
    }
    catch (error) {
        if (error instanceof ApiError_1.default)
            throw error; // Re-throw known validation/stock errors
        logContext.error = error;
        logger_1.default.error(`Error during POS checkout transaction`, logContext);
        // Provide a more context-specific error if possible
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, `Checkout failed: ${error.message || 'Internal Server Error'}`);
    }
};
/** Get details of a specific POS Session */
const getSessionById = async (sessionId, tenantId) => {
    const logContext = { function: 'getSessionById', sessionId, tenantId };
    try {
        const session = await config_1.prisma.posSession.findFirst({
        ...
        if (!session) {
            logger_1.default.warn(`POS Session not found or tenant mismatch`, logContext);
            return null;
        }
        logger_1.default.debug(`POS Session found successfully`, logContext);
        return session;
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error fetching POS Session by ID`, logContext);
        // --- FIX: Throw from catch ---
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve session.');
        // ---------------------------
    }
};
/** Query POS Sessions */
const querySessions = async (filter, orderBy, limit, page) => {
    const skip = (page - 1) * limit;
    const tenantIdForLog = typeof filter.tenantId === 'string' ? filter.tenantId : undefined;
    const logContext = { function: 'querySessions', tenantId: tenantIdForLog, limit, page };
    if (!tenantIdForLog) {
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Tenant context missing.');
    }
    try {
        const [sessions, totalResults] = await config_1.prisma.$transaction([
            config_1.prisma.posSession.findMany({
                where: filter,
                include: {
                    user: { select: { id: true, firstName: true, lastName: true } },
                    location: { select: { id: true, name: true } }
            ...
            config_1.prisma.posSession.count({ where: filter }),
        ]);
        logger_1.default.debug(`POS Session query successful, found ${sessions.length} of ${totalResults}`, logContext);
        return { sessions, totalResults };
    }
    catch (error) {
        logContext.error = error;
        logger_1.default.error(`Error querying POS sessions`, logContext);
        // --- FIX: Throw from catch ---
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, 'Failed to retrieve sessions.');
        // ---------------------------
    }
};
exports.posService = {
