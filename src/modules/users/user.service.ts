...
import { User, Prisma, Role, Location } from '@prisma/client'; // Import necessary Prisma types
...
// Helper type for user response (omitting password) including simplified roles and locations
export type SafeUserWithRoles = Omit<User, 'passwordHash'> & {
    roles: { role: Pick<Role, 'id' | 'name'> }[];
    locations: { location: Pick<Location, 'id' | 'name'> }[];
};
...
export type SafeUserWithRoleDetails = Omit<User, 'passwordHash'> & {
    ...
    locations: { location: Location }[]; // Include full location details
};
...
const createUser = async (userData: CreateUserDto, tenantId: string): Promise<SafeUserWithRoles> => {
    ...
    // 3. Check if provided locationIds are valid for the tenant
    ...
    if (userData.locationIds && userData.locationIds.length > 0) {
        const validLocations = await prisma.location.findMany({
            where: { id: { in: userData.locationIds }, tenantId: tenantId },
            select: { id: true }
        });
        if (validLocations.length !== userData.locationIds.length) {
            const invalidIds = userData.locationIds.filter(reqId => !validLocations.some(validLoc => validLoc.id === reqId));
            logContext.invalidLocationIds = invalidIds;
            logger.warn(`User creation failed: Invalid location ID(s) provided for this tenant`, logContext);
    ...
    try {
        const createdUserWithRoles = await prisma.user.create({
            data: {
                ...
                locations: { // Connect initial locations
...
const createUnassignedUser = async (userData: CreateUnassignedUserDto): Promise<SafeUserWithRoles> => {
    ...
    try {
        ...
        return { ...createdUser, roles: [], locations: [] } as SafeUserWithRoles;
    ...
    try {
        const user = await prisma.user.findFirst({ // Use findFirst for explicit tenant check
            ...
            select: { // Select fields, include full Role details here
                ...
                locations: { select: { location: true } } // Get full Location object
            },
        });
