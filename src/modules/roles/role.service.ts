...
import { Prisma, Role, Permission } from '@prisma/client'; // Ensure Permission is imported
...
const updateRoleById = async (roleId: string, updateData: UpdateRoleDto, tenantId: string): Promise<RoleWithPermissions> => {
    ...
    if (existing.isSystemRole) {
        logger.warn(`Update failed: Attempted to modify system role`, logContext);
    ...
    if (updateData.name && updateData.name !== existing.name) {
        ...
        if (nameExists) {
            ...
            throw new ApiError(httpStatus.CONFLICT, `Role name "${updateData.name}" already exists.`);
        }
    }
 ...
 */
const assignPermissionToRole = async (roleId: string, permissionId: string, tenantId: string): Promise<void> => {
    const logContext: LogContext = { function: 'assignPermissionToRole', roleId, permissionId, tenantId };
    ...
    try {
        ...
        logContext.error = error;
        logger.error(`Error assigning permission to role`, logContext);
        // Handle potential DB errors (e.g., foreign key constraint if ID was wrong despite checks)
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to assign permission.');
    }
};
 ...
 * @returns {Promise<void>}
 */
const removePermissionFromRole = async (roleId: string, permissionId: string, tenantId: string): Promise<void> => {
    ...
    try {
        ...
        if (deleteResult.count === 0) {
            logger.warn(`Permission assignment not found for role ${roleId} and permission ${permissionId}. No action taken.`, logContext);
            // Don't throw error, operation is idempotent
        } else {
            logger.info(`Permission ${permissionId} removed successfully from role ${roleId}`, logContext);
        }
        // Invalidate role cache if implemented
    } catch (error: any) {
        logContext.error = error;
        logger.error(`Error removing permission from role`, logContext);
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to remove permission.');
    }
};
 ...
 * @returns {Promise<void>}
 */
const addPermissionsToRole = async (roleId: string, permissionIds: string[], tenantId: string): Promise<void> => {
    const logContext: LogContext = { function: 'addPermissionsToRole', roleId, permissionIds, tenantId };

    if (!permissionIds || permissionIds.length === 0) {
        logger.info(`No permission IDs provided to add to role ${roleId}`, logContext);
    ...
    try {
        ...
        logContext.error = error;
        logger.error(`Error assigning permissions to role`, logContext);
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to assign permissions.');
    }
};
...
const removePermissionsFromRole = async (roleId: string, permissionIds: string[], tenantId: string): Promise<void> => {
    ...
    try {
        ...
        logger.info(`Attempted to remove ${permissionIds.length} permissions from role ${roleId}. Removed ${deleteResult.count} assignments.`, logContext);
        // Invalidate role cache if implemented

    } catch (error: any) {
        logContext.error = error;
        logger.error(`Error removing permissions from role`, logContext);
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to remove permissions.');
    }
};
